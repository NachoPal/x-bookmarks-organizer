 ...
 * untouched by the SDK and the provider SDKs it bundles.
 */
...
/** Hosted upstreams: each is a pi provider id, billed per token to the owner's key. */
type HostedUpstream = 'anthropic' | 'openai' | 'xai' | 'openrouter';
export type PiUpstream = HostedUpstream | 'local';

interface UpstreamInfo {
  label: string;
  /** The credential-chain key holding this upstream's API key. */
  keyName?: string;
}

export const PI_UPSTREAMS: Record<PiUpstream, UpstreamInfo> = {
  anthropic: { label: 'Anthropic API', keyName: 'ANTHROPIC_API_KEY' },
  openai: { label: 'OpenAI API', keyName: 'OPENAI_API_KEY' },
  xai: { label: 'xAI API', keyName: 'XAI_API_KEY' },
  openrouter: { label: 'OpenRouter', keyName: 'OPENROUTER_API_KEY' },
  local: { label: 'local OpenAI-compatible endpoint' },
};

const UPSTREAM_IDS = Object.keys(PI_UPSTREAMS) as PiUpstream[];
...
export function parseModelRef(ref: string): PiModelRef | undefined {
  ...
  const upstream = ref.slice(0, slash) as PiUpstream;
  const modelId = ref.slice(slash + 1).trim();
  if (!UPSTREAM_IDS.includes(upstream) || !modelId) return undefined;
  return { upstream, modelId };
}

function badRefMessage(ref: string): string {
  return (
    `pi-ai model "${ref}" is not "<upstream>/<model>". Upstreams: ${UPSTREAM_IDS.join(', ')} - ` +
    'e.g. "anthropic/claude-haiku-4-5", "openrouter/google/gemini-2.5-flash" or "local/llama3.1:8b".'
  );
}

/**
 * The models the settings selector offers. A short, curated list rather than
 * pi's full catalog (OpenRouter alone lists hundreds): any other pi model still
 * works by id through `XBOOKMARKS_TAXONOMY_MODEL` / `XBOOKMARKS_MODEL`.
 *
...
export const CURATED_PI_MODELS: readonly CuratedPiModel[] = [
  {
    ...
    role: 'Most capable Claude - the pick for designing the tree',
  ...
  },
  {
    ...
    price: { input: 0.05544, output: 0.11088 },
  },
];
...
function toProviderModel(m: CuratedPiModel): ProviderModel {
  ...
  return {
    ...
    description:
      `${m.role}. PAID: ${formatPrice(m.price.input)} in / ${formatPrice(m.price.output)} out ` +
      `per 1M tokens, ${compactTokens(m.contextWindow)} context` +
...
export interface PiRuntime {
  ...
  findModel(upstream: HostedUpstream, modelId: string): Model<Api> | undefined;
...
/**
 * Build the real runtime. Only the four hosted providers this app offers are
 * registered (not pi's `providers/all`, which would load ~40 catalogs), plus a
 * keyless OpenAI-compatible provider for a local endpoint.
 *
...
export async function loadPiRuntime(): Promise<PiRuntime> {
  const [core, anthropic, openai, xai, openrouter, completions] = await Promise.all([
  ...
  models.setProvider(xai.xaiProvider());
  models.setProvider(openrouter.openrouterProvider());
  models.setProvider(
    core.createProvider({
      id: 'local',
      name: 'Local endpoint',
  ...
  return {
    findModel: (upstream, modelId) => models.getModel(upstream, modelId),
...
/** Make a failure safe to show: no key of any upstream, bounded length. */
export function redactError(raw: string): string {
  return redactCliError(raw.replace(/xai-[A-Za-z0-9_-]{16,}/g, '[redacted]'));
}
...
export function createPiAiProvider(load: () => Promise<PiRuntime> = loadPiRuntime): ProviderDefinition {
  ...
  async function modelFor(
    ...
    const model = rt.findModel(parsed.upstream, parsed.modelId);
    if (!model) {
      return {
        health: {
          state: 'unconfigured',
  ...
  return {
    ...
    configKeys: [
      ...(['anthropic', 'openai', 'xai', 'openrouter'] as const).map((u) => ({
        key: PI_UPSTREAMS[u].keyName as string,
        required: false,
        description: `API key for ${PI_UPSTREAMS[u].label} models (PAID per token).`,
        secret: true,
      })),
      {
